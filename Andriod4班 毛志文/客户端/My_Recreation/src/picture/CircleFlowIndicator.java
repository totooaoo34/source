...
public class CircleFlowIndicator extends View implements FlowIndicator,
	...
	public CircleFlowIndicator(Context context, AttributeSet attrs) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				add.R.styleable.CircleFlowIndicator);

		// 变得不活跃的圆式,默认为“填补”
		int activeType = a.getInt(add.R.styleable.CircleFlowIndicator_activeType,
				STYLE_FILL);
		...
		// 得到一个自定义颜色
		int activeColor = a
				.getColor(add.R.styleable.CircleFlowIndicator_activeColor,
						activeDefaultColor);

		// 变得不活跃的圆式
		int inactiveType = a.getInt(
				add.R.styleable.CircleFlowIndicator_inactiveType, STYLE_STROKE);

		int inactiveDefaultColor = 0x44FFFFFF;
		int inactiveColor = a.getColor(
				add.R.styleable.CircleFlowIndicator_inactiveColor,
				inactiveDefaultColor);

		//检索半径
		radius = a.getDimension(add.R.styleable.CircleFlowIndicator_m_centered, 4.0f);

		circleSeparation = a.getDimension(
				add.R.styleable.CircleFlowIndicator_circleSeparation, 2 * radius
						+ radius);
		activeRadius = a.getDimension(
				add.R.styleable.CircleFlowIndicator_activeRadius, 0.5f);
		// 检索淡出时间
		fadeOutTime = a.getInt(add.R.styleable.CircleFlowIndicator_fadeOut, 0);

		mCentered = a.getBoolean(add.R.styleable.CircleFlowIndicator_m_centered,
				false);
