...
<html>
  ...
  <body>
	<h1>修改信息 ☞ <a href="fwxxAction!findAllFwxx">返回主页</a></h1>
	<h1></h1>
	
	<form action="fwxxAction!upByIdFwxx">
	<input style="display: none;" name="fwxx.fwid" value="${fwxx.fwid}">
		<table>
			<tr>
				<td class="title">标题：</td>
				<td><input name="fwxx.title" value="${fwxx.title }"/></td>
			</tr>
			<tr>
				...
				<td>
					...
					<SCRIPT type="text/javascript">
						var qxId = document.getElementsByName("qx.qxid")[0];
						for(var i = 0; i < qxId.length; i++){
							if(qxId[i].value==${qx.qxid}){
								qxId[i].selected="selected";
							}
						}
						//区县与街道的二级联动ajax
						function ajaxJd(){
			...
			<tr>
				...
				<td>
					<SELECT name="fwxx.jdid" id="jdid">
						<option value="0">-请选择-</option>
						<c:forEach items="${jds}" var="v">
							<option value="${v.jdid}">${v.jd}</option>	
						</c:forEach>
					</SELECT>
					<script type="text/javascript">
						var jdId = document.getElementsByName("fwxx.jdid")[0];
						for(var i = 0; i < jdId.length; i++){
							if(jdId[i].value==${fwxx.jdid}){
								jdId[i].selected="selected";
							}
						}
					</script>
				</td>
			...
			<tr>
				...
				<td>
					...
					<script type="text/javascript">
						var lxId = document.getElementsByName("fwxx.lxid")[0];
						for(var i = 0; i < lxId.length; i++){
							if(${fwxx.lxid}==lxId[i].value){
								lxId[i].selected="selected";
							}
						}
					</script>
				</td>
			...
			<tr>
				<td colspan="2" >
					<s:submit value="确认修改" align="center"></s:submit>
				</td>
			</tr>
		</table>
	</form>
